...
export const DoctorProfilePage = () => {
  ...
  const serviceHistory = appointments.filter((appointment) => appointment.doctor_id === doctorId);

  if (isLoading) {
    return (
      <div className="min-h-screen bg-surface flex items-center justify-center">
        <p className="text-body-md text-on-surface">Loading doctor profile...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-surface pb-24">
      <header className="bg-surface-container-lowest border-b border-outline-variant sticky top-0 z-10">
        <div className="px-6 py-4 flex items-center justify-between">
          ...
          <h1 className="text-headline-md font-bold text-primary">Detail Dokter</h1>
          <span className="w-10" />
        </div>
      </header>
      ...
      <main className="px-6 py-6 space-y-6">
        ...
        <section className="bg-surface-container-lowest border border-outline-variant rounded-xl p-5 space-y-4">
          <h3 className="text-headline-sm text-primary">Informasi Profesional</h3>
          <InfoLine label="Nama Dokter" value={doctor.name} />
          <InfoLine label="Riwayat Pendidikan" value={doctor.education ?? '-'} />
          <InfoLine label="Tanggal Mulai Praktik" value={formatDate(doctor.practice_start)} />
          <InfoLine label="Tanggal Masuk" value={formatDate(doctor.joined_at)} />
          ...
          <InfoLine label="Riwayat Tempat Praktek" value={doctor.practice_history?.join(', ') ?? '-'} />
        </section>

        <section>
          <h3 className="text-headline-sm text-on-surface mb-4">Riwayat Pelayanan</h3>
          <div className="space-y-4">
            {serviceHistory.length === 0 ? (
              <p className="text-body-md text-on-surface-variant">Belum ada riwayat layanan.</p>
            ) : (
                  ...
                  <Link
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <Avatar src={pet?.avatar_url} alt={pet?.name} size="md" initials={pet?.name.substring(0, 2).toUpperCase()} />
                        <div>
                          <h4 className="text-headline-sm text-on-surface">{pet?.name ?? 'Unknown Pet'}</h4>
                          <p className="text-body-sm text-on-surface-variant">{serviceLabels[appointment.service_type]}</p>
                        </div>
                      </div>
                    ...
                    <div className="border-t border-outline-variant mt-4 pt-4">
                      ...
                      <p className="text-body-md text-on-surface font-semibold">{formatDateTime(appointment.scheduled_date)}</p>
                    </div>
                  </Link>
                );
              })
