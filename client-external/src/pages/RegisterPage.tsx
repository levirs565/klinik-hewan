...
export const RegisterPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.registerOwner({
        email,
        password,
        full_name: fullName,
        phone_number: '',
      ...
      <div className="hidden md:flex md:w-1/2 relative bg-surface-container-low items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg p-section-gap">
          <div className="flex items-center gap-base mb-container-margin">
            ...
            <span className="font-display-lg text-display-lg text-primary">VetConnect</span>
          </div>
          ...
          <p className="font-body-lg text-body-lg text-on-surface-variant mb-section-gap">
            Join our network to manage your pet's health records, schedule appointments seamlessly, and connect with top-tier veterinary professionals.
          </p>
          <div className="inline-flex items-center gap-base bg-surface-container-lowest/80 backdrop-blur-md border border-outline-variant rounded-full px-4 py-2">
            <div className="flex -space-x-2">
              <div className="w-8 h-8 rounded-full bg-tertiary-fixed border-2 border-surface-container-lowest flex items-center justify-center text-on-tertiary-fixed text-xs font-bold">V1</div>
              <div className="w-8 h-8 rounded-full bg-secondary-fixed border-2 border-surface-container-lowest flex items-center justify-center text-on-secondary-fixed text-xs font-bold">V2</div>
              <div className="w-8 h-8 rounded-full bg-primary-fixed border-2 border-surface-container-lowest flex items-center justify-center text-on-primary-fixed text-xs font-bold">+</div>
            </div>
            <span className="font-label-sm text-label-sm text-on-surface-variant ml-2">Trusted by 500+ Clinics</span>
          </div>
        </div>
      </div>
      ...
      <div className="w-full md:w-1/2 min-h-screen flex items-center justify-center p-container-margin sm:p-section-gap bg-surface-bright">
        <div className="w-full max-w-[480px]">
          <div className="md:hidden flex items-center gap-base mb-container-margin justify-center">
            ...
            <span className="font-headline-md text-headline-md font-bold text-primary">VetConnect</span>
          </div>
          <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-container-margin sm:p-section-gap shadow-sm">
            <div className="mb-container-margin">
              <h1 className="font-headline-lg text-headline-lg text-on-surface mb-base">Create an account</h1>
              <p className="font-body-md text-body-md text-on-surface-variant">Enter your details to register as a new pet owner.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-gutter">
              <div className="flex flex-col">
                <label className="font-label-md text-label-md text-on-surface-variant mb-2" htmlFor="fullName">
                ...
                />
                {errors.fullName && <p className="mt-2 text-error text-body-sm">{errors.fullName}</p>}
              </div>

              <div className="flex flex-col">
                <label className="font-label-md text-label-md text-on-surface-variant mb-2" htmlFor="email">
                ...
                <input
                  ...
                  className="w-full bg-surface-bright border border-outline-variant rounded-lg px-4 py-3 font-body-md text-body-md text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-colors"
                />
                {errors.email && <p className="mt-2 text-error text-body-sm">{errors.email}</p>}
              </div>

              <div className="flex flex-col">
                <label className="font-label-md text-label-md text-on-surface-variant mb-2" htmlFor="password">
                ...
                {errors.password && <p className="mt-2 text-error text-body-sm">{errors.password}</p>}
              </div>

              <div className="flex flex-col mb-base">
                <label className="font-label-md text-label-md text-on-surface-variant mb-2" htmlFor="confirmPassword">
                ...
                {errors.confirmPassword && <p className="mt-2 text-error text-body-sm">{errors.confirmPassword}</p>}
                <p className="font-body-sm text-body-sm text-on-surface-variant mt-2">Must be at least 8 characters long.</p>
              </div>
              ...
              {errors.terms && <p className="text-error text-body-sm mb-base">{errors.terms}</p>}
            ...
            <div className="mt-container-margin text-center pt-container-margin border-t border-outline-variant/30">
              <p className="font-body-md text-body-md text-on-surface-variant">
                ...
                <Link to="/login" className="text-primary font-bold hover:underline transition-colors ml-1">
                  Login here
                </Link>
              </p>
            </div>
